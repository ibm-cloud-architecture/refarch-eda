...
!!! note
    ...
    * Java 1.8 .mdx microprofile architecture
    * Python 3.6
    ...
    * Apache Kafka, Kafka API
    ...
    We have build a getting started and tutorial list for you to study [here](core-techno-getstarted.mdx).
...
=======
* Understand the [Key EDA concepts](../concepts/README.mdx) like events, event streams, events and messages differences... 
>>>>>>> fix broken links
...
* As kafka is the event backbone, review its [key concepts and building blocks](../kafka/readme.mdx) and then review how to support [high availability (HA) and disaster recovery (DR) with IBM Event Streams or Kafka Architecture Considerations](../kafka/arch.mdx).
...
See [this note](../deployments/eventstreams//es-ibm-cloud.mdx) for creating Event Stream with Enterprise plan.
...
=======
* We are proposing a [set of activities](../methodology/readme.mdx) to develop and release an event-driven solution based on the agile development approach described in the IBM Garage method. 
* The adopted approach for event identification is the [Event storming methodology](https://ibm-cloud-architecture.github.io/refarch-eda/methodology/readme/) introduced by Alberto Brandolini in "Introducing event storming book". We have extended it with the event insight practice used to identify potential real time analytics use cases.
* Once the event storming deliver events, commands, aggregates we can start doing some [Domain Design Driven](../methodology/ddd.mdx) exercises and apply it to the business application.
>>>>>>> fix broken links
...
First be sure to complete the pre-requisites by following [those steps](ttps://ibm-cloud-architecture.github.io/refarch-kc/pre-requisites.mdx).
...
=======
* Read [Event driven design patterns for microservice](../design-patterns/ED-patterns.mdx) with the Command Query Responsability Segregation, event sourcing and saga patterns. 

* Review the [Event sourcing design pattern explanations](../design-patterns/event-sourcing.mdx) and how it is tested with some integration tests: 
>>>>>>> fix broken links

* Review the [CQRS pattern](../design-patterns/cqrs.mdx).
...
* [Our Kubernetes troubleshooting notes](https://github.com/ibm-cloud-architecture/refarch-integration/blob/master/docs/icp/troubleshooting.mdx)

* [Kafka monitoring](../kafka/monitoring.mdx)
...
* Read introduction to [act on events with IBM Cloud Functions](../evt-action/README.mdx)
