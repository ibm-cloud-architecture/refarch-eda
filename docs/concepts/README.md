...
* Ones where we connect to a real time event stream, e.g. from an IOT device, a Voice Stream from a telephone system, a Video stream, Ship/Plane locations from global positioning systems.
...
* **Events:** Events are persisted as a replay-able Stream History, event consumers are not tied to the producer, an event is a record of something which has happened and so can't be changed (you can't change history)
