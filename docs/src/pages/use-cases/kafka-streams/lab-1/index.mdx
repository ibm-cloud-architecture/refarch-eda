...
- Create the directory structure you will need for your Java file. _(**NOTE:** If you are working in an IDE, this may be done for you when you create your package and classes.)_
   ...
   ```mkdir -p src/test/java/com/ibm/garage/cpat```

- Create a new file named `src/test/java/com/ibm/garage/cpat/TestLoadKtableFromTopic.java`.
...
- What the above code does is it uses TopologyTestDriver to mimic a Topology. A Topology is basically a graph of stream processors (nodes) and the edges between these nodes are the streams. In the first section we instantiate our `TopologyTestDriver` named `testDriver`, as well as the topic name and store name.
...
- Let's create a new class for our Plain Old Java Object (POJO) named FinancialMessage: `src/main/java/com/ibm/garage/cpat/domain/FinancialMessage.java`
...
- Now that we have our new Java class, let's create a new and separate Java Test class: `src/test/java/com/ibm/garage/cpat/lab/TestFinancialMessage.java`.
