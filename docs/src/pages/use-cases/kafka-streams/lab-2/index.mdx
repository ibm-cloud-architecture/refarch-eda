...
## Overview
- This is a continuation of the previous [Lab 1](/use-cases/kafka-streams/lab-1/. You can follow most of those steps before you get started here.
...
along the lines of `src/main/java/com/ibm/Infrastructure/MockProducer.java`
...
`src/main/java/com/ibm/Domain/FinancialMessageTopology.java`
...
isn't needed, so you can comment out those two lines as well as not needing the `CERT_LOCATION` ENV. Also if you would like to connect to a plain local Kafka Cluster on a container for example you will need to remove some of the security settings in `application.properties` (ssl, sasl, etc.)
