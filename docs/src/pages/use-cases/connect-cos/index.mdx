...
1. Deploy to your OpensHift project. Modify the configmap to point to your Bootstrap URL

  ```sh
  oc apply -k kustomize/
  ```
1. Update the `applications.properties` file if you are not connect to Event Streams via PLAINTEXT security protocol.
...
1. See the README to run the producer application locally with quarkus dev
