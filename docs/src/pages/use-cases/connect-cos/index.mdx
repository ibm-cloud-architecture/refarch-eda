...
- We will need to create a single `INBOUND` topic. The `INBOUND` topic is where our Quarkus application will produce to and where our IBM Cloud Object Storage Sink Connector will pull data from to send to the COS Bucket.
...
Take note on the line that says @Outgoing("INBOUND"). For the purposes of this use case, we will use the INBOUND topic name that we created in the Event Streams Topic step earlier. Replace whatever is inside the quotation marks.
...
- Run the producer code by running either of the following commands
...
or
```bash
java -jar target/quarkus-kafka-1.0-SNAPSHOT-runner.jar
```
...
*Note* - Like the above Kafka Connect Cluster section, these steps are based on Event Streams v2019.4.x, there is an  [Event Streams V10](#event-streams-v10-addendum) section if that is your environment.
...
**(IMPORTANT) The Strimzi Kafka Connect Cluster uses a Java 8 runtime so make sure you're actively using the Java 8 JRE at least for building the connector JAR file, otherwise the Kafka Connect Cluster will not be able to detect the COS Sink Connector.**
...
- We will need *both* the `External` and `Internal` connection details.

*Event Streams v10 Credentials*

- For connecting external applications like our Quarkus producer application will need the `External Connection` details which involve `SCRAM Credentials` whereas our Event Streams Kafka Connect Cluster requires `Internal Connection` and TLS certificates.
...
- Select a name for your secret and make sure you remember/take note of it as we will need this later. Also choose the Produce, Consume, Create Topics and Schema Option.
...
- You will need `Internal Connection` details and the created `TLS Credentials` for the Kafka Connect Cluster.
...
- Apply this yaml like in the original steps.
```shell
...
- Kick off a new build just like in the previous Event Streams version.
```
oc start-build connect-cluster-101-connect --from-dir ./connectors/ --follow
```
