...
<strong>Updated 1/25/2022</strong>
</InlineNotification>
...
We propose to use one GitOps Catalog to centralize the Operator definitions with Kustomize overlays. An example of such catalog is the [eda-gitops-catalog](https://github.com/ibm-cloud-architecture/eda-gitops-catalog).
Each operator is defined with the subscription manifest and then update to change the product version. Here is an example for Event Streams:
...
the SAGA pattern implemented with MQ
* [store-mq-gitops](https://github.com/ibm-cloud-architecture/store-mq-gitops): real-time inventory demo with MQ kafka connector and Event Streams