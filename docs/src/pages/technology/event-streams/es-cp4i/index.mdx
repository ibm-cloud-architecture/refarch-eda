...
for that follow the simple [instructions here](https://www.ibm.com/docs/en/cpfs?topic=311-installing-foundational-services-by-using-console).

Create also the ibm-entitlement-key in the `ibm-common-services`  namespace.
...
Those are the deployment the foundational services are creating:
...
Recalls that operands are what operators managed. 
...
those services were not present before.

Here is the updated list of deployments:
...
So it could take some time to get this up and running.

It may take some time to start all needed dependencies and Event Stream pods. The result of this cluster creation should looks like: 
