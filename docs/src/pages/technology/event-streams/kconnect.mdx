...
We assume that you have an instance of Event Streams already running on IBM Cloud with a set of manager-level credentials created.  The credentials will come in the form of a JSON document as seen in the previous section.
...
In this scenario we are using another connector from [IBM messaging github: source connector for RabbitMQ](https://github.com/ibm-messaging/kafka-connect-rabbitmq-source). The configuration for the connector is also done in json, with a POST to the Kafka connectors URL.

The [lab repository](https://github.com/jbcodeforce/eda-kconnect-lab) includes a docker compose file (`RabbitMQ-Kconnect-compose.yaml`) under the `infrastructure` folder. 

In scenario 1, the we were using the official IBM MQ Connector docker image, but in this scenario we have to define a Kafka connect image that includes also the RabbitMQ and JDBC connector. Which means we needs to get the different connector jar files, and build a docker image. Those steps were described in [previous section](#scenario-setup).

### Start the backend environment

In this section we start RabbitMQ, Kafka Connect and the Store Sale generator app.
...
Go to the Queue tab and add `items` queue:
