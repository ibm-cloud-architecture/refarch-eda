...
<AnchorLinks>
  ...
  <AnchorLink>Integrating with automation products</AnchorLink>
...
## Integrating with automation products

EDA is becoming the new Service Oriented Architecture as a foundation to expose business service, and business entity states.

A new set of automation products like Robot Process Automation bots, Process Mining, Chat Bots, and more traditional workflow and decision engines
are leveraging the messaging as a service as presented in previous section.

So these lead to a new digital automation reference architecture, based on event-driven and messaging communication:

![](./images/automation-eda.png)

On the top row the capabilities are:

*  Existing business applications, and SOA services: most of them are J2EE applications running on application servers, 
some are legacy based using transactions. It will be easy for those applications to integrate new microservices, or webapp
exposing JSON / RESTful resources via an API Gateway.
* New single page applications are also accessing business services via gateway, and use HTTP (RESTful) protocol. Those single page applications 
can be developed using IBM 
* Existing business process applications may have been designed to be triggered by message arrival, and may be able to emit intermediate
events. IBM BPM, for example, has JMS queue integration to consume and publish messages.
...
they ask someone to participate into their business process. And they publish states change via events to pub/sub topics.
* Consumers can use Event Gateway to access to topics and process the data, do data mapping and data transformation for long persistence into data lake.
* The key integration between EDA and automation is by adding stateful agents that are aggregating, correlating, 
and computing the next best actions. Those actions can be to trigger a workflow, initiate a RPA bot, call
one or more decision services. Those stateful agents can use complex event processing engines, rule engines, and 
integrate with predictive scoring.
...
* The AI workbench includes tools to do data analysis and visualization, build training and test sets from any datasource and in particular topics, and develop models. Models are integrated to streaming analytics component.
