...
Robot Process Automation bots, Process Mining, Chat Bots, and more traditional workflow and decision engines.
...
On the top row, the capabilities group:
...
* New single page applications are also accessing business services via the gateway, and use HTTP (RESTful) protocol. Those single page applications 
...
events. IBM BPM, for example, has JMS queue integration to consume and publish messages to JMS queues, which should be defined in IBM MB Brokers.
