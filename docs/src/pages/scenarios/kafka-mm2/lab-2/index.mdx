...
* Clone the github to get access to the mirror maker 2 manif we are using:
...
In this lab, Mirror Maker 2 will run on the same cluster as Event Streams within the same namespace. 
...
* Verify the chareacteristics of the Mirror Maker 2 instance using the CLI
 ...
 ```
* As Mirror Maker 2 is set to  be connected to the `es-1-cluster` which is the event streams on OCP, it will create its internal topic as:
