...
<strong>Updated 12/12/2021 - still working progress</strong>
</InlineNotification>


This chapter describes how event driven architecture addresses the implementation of reactive systems and 
presentsthe most recent technologies to implement such event-driven responsive solutions.

<AnchorLinks>
    <AnchorLink>Overview</AnchorLink> Cloud native, distributed systems, reactive systems...
    <AnchorLink>EDA and reactive systems</AnchorLink> 
    <AnchorLink>Technology review</AnchorLink> Vert.x, reactive messaging, mutiny, AMQP, kantive eventing...
...
as recently demonstrated by AWS outtage.  Kubernetes helps delivering an abstraction on top
of cloud providers, and use them as infrastructure as a services, and bring your own platform.
...
out of the three guarantees of Consistency, Available Partition tolerance. 
...
Quarkus/Vert.x has a router that can be intercepted to simulate communication loss, wrong response or delay. 
...
idempotence: multiple request with same content, results in the same state and same output.
Idempotence can be implemented using unique identifiers. But server applications need to keep
state of those ids, which means using Storagem which is under the CAP theorem. We may
...
Modern solution based on microservices need to support load and failure and developers 
adopt the reactive manifesto and modern programming libraries and software to support 
...
With messaging applications can scale horizontally giving the elastic need of the reactive manifesto.
...
access I/Os on a two CPUs machine. When the second thread starts working on the IO the CPU is locked 
and now CPU yellow is supporting 2 threads (1 amd 3)
...
Non blocking IO framework or library adopts the `reactor pattern` where requests are now internal 
...
With non-blocking IO the IO operation can be allocated to another pool and the allocation of CPU 
to thread is well balanced: 

 ![6](./images/conc-2.png)

Vert.X is the open source library to build such non-blocking IO app, and using `vertices` to support 
